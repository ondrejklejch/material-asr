...
void RemoveDisambiguationSymbols(kaldi::KwsLexicographicFst &index,
...
class KwsSearchClass {
  public:
    KwsSearchClass(const std::string key,
                   fst::VectorFst<fst::StdArc> keyword,
                   const kaldi::KwsLexicographicFst &index,
                   const unordered_map<uint32, uint64> &label_decoder,
                   const std::string &stats_wspecifier,
                   VectorOfDoublesWriter *stats_writer,
                   VectorOfDoublesWriter *result_writer,
                   double keyword_beam,
                   int32 n_best,
                   int32 keyword_nbest,
                  ...
                  stats_wspecifier(stats_wspecifier),
                  stats_writer(stats_writer),
                  result_writer(result_writer),
                  keyword_beam(keyword_beam),
                  n_best(n_best),
                  keyword_nbest(keyword_nbest),
    ...
    ~KwsSearchClass() {
      ...
      if (final_result_fst.Start() == kNoStateId)
        return;
      ...
      for (ArcIterator<KwsLexicographicFst>
            aiter(final_result_fst, final_result_fst.Start()); !aiter.Done(); aiter.Next()) {
        const KwsLexicographicArc &arc = aiter.Value();

        // We're expecting a two-state FST
        if (final_result_fst.Final(arc.nextstate) != Weight::One()) {
        ...
        uint64 osymbol = label_decoder.find(arc.olabel)->second;
        uid = kaldi::DecodeLabelUid(osymbol);
    ...
    void operator () () {
      ...
      final_result_fst = result_fst;
      Project(&final_result_fst, PROJECT_OUTPUT);
      Minimize(&final_result_fst, (KwsLexicographicFst *) nullptr, kDelta, true);
      ShortestPath(final_result_fst, &result_fst, n_best);
      RmEpsilon(&final_result_fst);
    };
  ...
  private:
    ...
    // Interim result fsts
    kaldi::KwsLexicographicFst result_fst, final_result_fst;
...
int main(int argc, char *argv[]) {
  try {
    ...
    timer.Reset();
    TaskSequencer<KwsSearchClass> sequencer(sequencer_config);
    ...
    unordered_map<uint64, uint32> label_encoder;
    unordered_map<uint32, uint64> label_decoder;
    KALDI_LOG << "TIMER: Loading index and IO took " << timer.Elapsed() << "s";

    timer.Reset();
    RemoveDisambiguationSymbols(index, label_encoder, label_decoder);
    KALDI_LOG << "TIMER: Removing disambiguation symbols took " << timer.Elapsed() << "s";
    ...
    for (; !keyword_reader.Done(); keyword_reader.Next()) {
      ...
      KwsSearchClass *task = new KwsSearchClass(
        key, keyword, index, label_decoder, stats_wspecifier, &stats_writer,
        &result_writer, keyword_beam, n_best, keyword_nbest,
        frame_subsampling_factor, negative_tolerance, &n_done, &n_fail);
    ...
    KALDI_LOG << "TIMER: KWS took " << timer.Elapsed() << "s";
