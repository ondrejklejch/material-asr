...
$cmd JOB=1:$nj $kwsdir/log/search.JOB.log \
  kws-search-parallel --num-threads=$num_threads --strict=$strict --negative-tolerance=-1 \
  "ark:gzip -cdf $indices_dir/index.JOB.gz|" ark:$keywords \
  "ark,t:|python process_results.py $kwsdatadir/keywords_id $kwsdatadir/utter_id $kwsdatadir/segments 3 $kwsdir/result.JOB.txt" \
  "" || exit 1;
